...
export default function Header() {
  ...
  // 3. Función para aplicar estilos si la ruta coincide
  const linkStyle = (path: string) => {
    const active = pathname === path;
    return `text-sm font-medium transition-all duration-200 pb-1 border-b-2 ${
      active
        ? "text-primary border-primary dark:text-white dark:border-primary" // Activo: Texto resaltado y borde azul
        : "text-slate-600 border-transparent hover:text-primary dark:text-slate-400 dark:hover:text-white" // Inactivo
    }`;
  };
      ...
      <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4 lg:px-10">
        <div className="flex items-center gap-12">
          <Link href="/" className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-bold tracking-tight text-slate-900 dark:text-slate-100 font-headline">
              VANGUARD
            </h1>
          </Link>

          <nav className="hidden items-center gap-8 md:flex">
            {/* 4. Aplicamos la función linkStyle a cada Link */}
            <Link href="/tienda" className={linkStyle("/tienda")}>
              Tienda
            </Link>
            <Link href="/descubrir" className={linkStyle("/descubrir")}>
              Descubrir
            </Link>
            <Link href="/biblioteca" className={linkStyle("/biblioteca")}>
              Biblioteca
            </Link>
          </nav>
        </div>

        {/* Lado Derecho (se mantiene igual) */}
        <div className="flex items-center gap-6">
          <div className="relative hidden sm:block">
            <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-xl">
              search
